...
import { getCollectionsList } from "@/lib/shop";
...
export default async function DropsPage({
  ...
  const collections = await getCollectionsList(lang);
        ...
        {collections.length === 0 ? (
          <Reveal>
            <div className="border-y border-line py-32 text-center">
              ...
              <p className="display mt-6 text-4xl italic text-mist">
                Henüz drop yok
              </p>
              <p className="mt-4 text-sm text-mist">
                İlk koleksiyon yakında.
              </p>
            </div>
          </Reveal>
        ) : (
          collections.map((c, i) => {
            ...
            const isUrl = c.heroImageUrl?.startsWith("http");
            return (
              <Reveal key={c.slug} delay={i * 0.1}>
                ...
                >
                  <div
                    ...
                    style={
                      isUrl
                        ? { backgroundImage: `url('${c.heroImageUrl}')` }
                        : { background: heroBg }
                    }
                  />
